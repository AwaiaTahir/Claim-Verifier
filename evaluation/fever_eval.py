"""
FEVER Dev Set Evaluation
Samples 100 claims from the FEVER labelled_dev split and evaluates the
claim verification system against ground-truth labels.

Label mapping:
  FEVER SUPPORTS       -> SUPPORTED
  FEVER REFUTES        -> REFUTED
  FEVER NOT ENOUGH INFO -> NOT ENOUGH EVIDENCE
"""
...
def load_fever_dev_claims(sample_size: int = SAMPLE_SIZE, seed: int = RANDOM_SEED) -> list[dict]:
    ...
    try:
        ...
        print(f"[Eval] Valid labeled claims: {len(valid)}")
        ...
        print("[Eval] Label distribution in dev set:")
        ...
        rng.shuffle(sampled)
        print(f"[Eval] Sampled {len(sampled)} claims for evaluation")
        return sampled
...
def run_fever_eval():
    ...
    print("\n" + "=" * 72)
    print("FEVER DEV SET EVALUATION RESULTS")
    print("=" * 72)
    ...
    if failures:
        ...
        for r in failures[:10]:  # Show first 10
            print(f"  Expected {r['expected']:25s} | Got {r['predicted']:25s} | {r['claim'][:60]}")

    # Save results to JSON
    out_path = ROOT_DIR / "evaluation" / "fever_eval_results.json"
